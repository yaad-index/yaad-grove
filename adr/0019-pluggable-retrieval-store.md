...
3. **`Enumerate` + the `kb_enumerate` tool** — fixes the under-recall query class (the whole point). Demonstrated on "which episodes cover game X".
   ...
   **Surface-form / alias resolution (invariant).** Normalization handles *format* drift but not *cross-script / cross-language* drift: `آیریش گیج` (a Persian transliteration) never normalizes to `irish gauge`, so a Persian user's query would miss the entity entirely. So an entity has **many surface forms mapping to one identity**: a canonical name plus aliases/transliterations (`Irish Gauge` ← `آیریش گیج`, `آیریش‌گیج`). `Store.Index` registers every surface form's normalized key against the same entity; `Enumerate` resolves **any** surface form → the entity → its documents. This is generic — a Berlin instance needs it for `Kreuzberg`/`کرویتسبرگ` — and it is the natural shape for the graph backend (alias nodes → canonical entity node). **Source of aliases:** an instance declares them in frontmatter (a per-note `aliases:` / `name_fa:` field authored by a native speaker — same "native speaker authors the data" principle as language packs), optionally seeded from an existing resolution map. Without an alias, only the canonical form resolves — aliases are additive, never required for the base case.
