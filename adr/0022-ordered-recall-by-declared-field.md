...
**3. Route recency / ordinal / count questions to it.** Widen the structured-tool trigger in the **deployed** prompt template (which today forces `kb_enumerate` for "which / list all / how many") to also cover **"latest / most recent / newest / oldest / first / last / highest-numbered / which is the Nth"**. 🔑 **Count phrasings are already steered and need nothing: the gap is recency and ordinal alone.** ⚠️ **This part does not ship with the code.** The engine default and the deployed template are two different files and only the deployed one routes anything, so a merged implementation leaves the routing inert until that template is edited in the repository it is deployed from — and a host-side edit does not survive the deploy. Similarity stays the default for "tell me about X"; the ordered/enumerate path is mandatory for order- and count-bearing questions, because those are unanswerable from a sample by construction.
